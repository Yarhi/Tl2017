...
int main ( int argc, char **argv )
{
   ...
   // fa_remove_transition(autom,0,'a',1);
   
   // suppresion d'un état
   // fa_remove_state(autom,4);

   // afficher nombre de transitions
   printf("%zu\n",fa_count_transitions(autom));

   // tester si autom est déterministe
   if (fa_is_deterministic(autom)) printf("Automate déterministe\n");
   else printf("Automate non déterministe\n");

   // tester si autom est complet
   if (fa_is_complete(autom)) printf("Automate complet\n");
